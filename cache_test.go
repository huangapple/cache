...
func TestIncrement(t *testing.T) {
	tc := New(DefaultExpiration, 0)
	n, err := tc.Increment("foo", 1, DefaultExpiration)
	if err != nil {
		t.Error("Couldn't Increment foo ")
	}
	if n != 1{
		t.Error("Increment is error")
	}

	n, err = tc.Increment("foo", -2, DefaultExpiration)
	if err != nil {
		t.Error("Couldn't Increment foo ")
	}
	if n != -1{
	...
	n, err = tc.Increment("foo", -10, DefaultExpiration)
	if err != nil {
		t.Error("Couldn't Increment foo ")
	}
	if n != -11{
...
func BenchmarkIncrement(b *testing.B) {
	...
	b.StartTimer()
	for i := 0; i < b.N; i++ {
		tc.Increment("foo", 1, DefaultExpiration)
	}
}
